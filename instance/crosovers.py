...
@njit
def get_final_value_from_mapping(mapped_values_0: np.ndarray, mapped_values_1: np.ndarray, value: int) -> int:
...
class PMX(BaseCrossover):
    ...
    def crossover(self, solution_a: np.ndarray, solution_b: np.ndarray) -> list[np.ndarray]:
        total_length = solution_a.shape[0]
        cut_a = random.randint(0, total_length - 3)
        cut_b = random.randint(cut_a + 1, total_length)
        solution_1, solution_2 = self._alter_solutions(solution_a, solution_b, cut_a, cut_b)
    ...
    def _alter_solutions(
        ...
        for jj in range(total_length):
            ...
            if solution_a[jj] in mapping_b:
                new_solution_a[jj] = get_final_value_from_mapping(mapping_b, mapping_a, solution_a[jj])
            else:
                new_solution_a[jj] = solution_a[jj]

            if solution_b[jj] in mapping_a:
                new_solution_b[jj] = get_final_value_from_mapping(mapping_a, mapping_b, solution_b[jj])
            else:
